...
export function handleKeyHint(
	...
	const handleFocusOut = (event: FocusEvent) => {
		if (!node.contains(event.relatedTarget as Node | null)) remove();
	};

	node.addEventListener('focusin', add);
	node.addEventListener('focusout', handleFocusOut);
	if (node.contains(document.activeElement)) add();
	...
	return {
		...
		destroy() {
			remove();
			node.removeEventListener('focusin', add);
